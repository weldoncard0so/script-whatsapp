async function enviarScript(scriptText) {
    while (true) {
        ...
        for (const line of lines) {
            ...
            const randomTimeout = Math.floor(Math.random() * 6001) + 5000;
            console.log("Tempo aleatório sorteado: ", randomTimeout);
            if (lines.indexOf(line) !== lines.length - 1) await new Promise(resolve => setTimeout(resolve, randomTimeout));
        }
        
        // Aguarda um período antes de reiniciar o loop
        await new Promise(resolve => setTimeout(resolve, 5000)); // Espera 5 segundos antes de reiniciar
    }
}
